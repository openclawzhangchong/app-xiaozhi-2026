...
class XiaozhiService {
  final Logger logger = Logger('XiaozhiService');
  // 单例实例
  ...
  Future<void> switchToVoiceCallMode() async {
    ...
    try {
      ...
      logger.info('已切换到语音通话模式');
    } catch (e) {
      logger.severe('切换到语音通话模式失败: $e');
      rethrow;
    }
  }
  ...
  Future<void> switchToChatMode() async {
    ...
    try {
      ...
      logger.info('已切换到普通聊天模式');
    } catch (e) {
      logger.severe('切换到普通聊天模式失败: $e');
      _isVoiceCallActive = false;
    }
  }
  ...
  Future<void> _init() async {
    // 使用配置中的MAC地址作为设备ID
    logger.info('初始化完成，使用MAC地址作为设备ID: $macAddress');
      ...
      void onceListener(XiaozhiServiceEvent event) {
        if (event.type == XiaozhiServiceEventType.textMessage) {
          ...
          if (event.data == message) {
            logger.info('忽略echo消息: ${event.data}');
            return;
          }

          logger.info('收到服务器响应: ${event.data}');
          ...
          logger.severe('收到错误响应: ${event.data}');
  ...
  Future<void> connectVoiceCall() async {
    try {
      ...
      logger.info('正在连接 $websocketUrl');
      logger.info('设备ID: $macAddress');
      logger.info('Token启用: true');
  ...
  Future<void> startSpeaking() async {
    try {
      ...
      logger.info('已发送开始说话消息');
    } catch (e) {
      logger.severe('开始说话失败: $e');
    }
  }
  ...
  Future<void> stopSpeaking() async {
    try {
      ...
      _webSocketManager?.sendMessage(jsonEncode(message));
      logger.info('已发送停止说话消息');
    } catch (e) {
      logger.severe('停止说话失败: $e');
    }
  }
  ...
  void _sendListenMessage() async {
    try {
      ...
      logger.info('已发送listen消息');

      // 开始录音
      _isVoiceCallActive = true;
      await AudioUtil.startRecording();
    } catch (e) {
      logger.severe('发送listen消息失败: $e');
      _dispatchEvent(
  ...
  Future<void> startListeningCall() async {
    try {
      // 确保已经有会话ID
      if (_sessionId == null) {
        logger.warning('没有会话ID，无法开始监听，等待会话ID初始化...');
        // 等待短暂时间，然后重新检查会话ID
        await Future.delayed(const Duration(milliseconds: 500));
        if (_sessionId == null) {
          logger.severe('会话ID仍然为空，放弃开始监听');
          throw Exception('会话ID为空，无法开始录音');
        }
      }

      logger.info('使用会话ID开始录音: $_sessionId');
      ...
      _webSocketManager?.sendMessage(jsonEncode(message));
      logger.info('已发送开始监听消息 (语音通话模式)');
    } catch (e) {
      logger.severe('开始监听失败: $e');
  ...
  Future<void> stopListeningCall() async {
    try {
      ...
      if (_sessionId != null && _webSocketManager != null) {
        ...
        _webSocketManager?.sendMessage(jsonEncode(message));
        logger.info('已发送停止监听消息 (语音通话模式)');
      }
    } catch (e) {
      logger.severe('停止监听失败: $e');
    }
  }
  ...
  void _handleTextMessage(String message) {
    ...
    try {
      ...
      switch (type) {
        ...
        default:
          // 对于其他类型的消息，直接忽略
          logger.warning('收到未知类型消息: $type, 原始数据: $message');
      }
    } catch (e) {
      logger.severe('解析消息失败: $e, 原始消息: $message');
    }
  }
  ...
  void _startCall() {
    try {
      ...
      logger.info('已发送开始通话消息');
    } catch (e) {
      logger.severe('开始通话失败: $e');
    }
  }
  ...
  Future<void> sendAbortMessage() async {
    try {
      if (_webSocketManager != null && _isConnected && _sessionId != null) {
        ...
        logger.info('发送中断消息: $abortMessage');
